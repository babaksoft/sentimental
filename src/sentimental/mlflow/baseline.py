...
def evaluate(df: pd.DataFrame, run_name: str, clean: bool = False):
    with mlflow.start_run(run_name=run_name) as run:
        ...
        params.update({f"tfidf_{key}": val for key, val in tfidf_params.items()})
        ...
        params.update({f"cv_{key}": val for key, val in cv_params.items()})
        ...
        params.update({f"model_{key}": val for key, val in model_params.items()})

        y = df[config.TARGET]
        if clean:
            x = df.drop(config.TARGET, axis=1)
            pipeline = Pipeline(
                [("transformer", build_pipeline()), ("classifier", model)]
            )
        else:
            x = df[config.FEATURE]
        ...
        start = datetime.now()
        cv_result = cross_validate(pipeline, x, y, scoring=scoring, cv=cv, n_jobs=-1)
        end = datetime.now()
